...
class JsonService {
  ...
  submitOrder(order) {
    ...
    // Find session
    const sessionIndex = data.activeSessions.findIndex(
      (s) => s.id === order.sessionId,
    );
    if (sessionIndex === -1) {
  ...
  exportOrdersToExcel(sessionId) {
    ...
    const selfPayTotal = orders.reduce(
      (sum, order) => (order.isSelfPay ? sum + (order.price || 0) : sum),
      0,
    );
